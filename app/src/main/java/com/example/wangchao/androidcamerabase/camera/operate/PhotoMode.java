...
import com.example.wangchao.androidcamerabase.camera.compare.CompareSizeByArea;
 ...
 * 拍照操作类
 */
public class PhotoMode extends BaseCamera2Operator {
    ...
    public void writePictureData(Image image) {
        if (mCamera2ResultCallBack != null) {
            mCamera2ResultCallBack.callBack(ObservableBuilder.createWriteCaptureImage(appContext, image));
        }
    }
    ...
    private void createCameraPreviewSession() {
        try {
            ...
            Log.d("camera_log","mPreviewSize.getWidth()==="+mPreviewSize.getWidth()+"   mPreviewSize.getHeight()="+mPreviewSize.getHeight());
    ...
    public void notifyFocusState() {
        if (mPreviewRequestBuilder != null) {
            Log.i("notifyFocusState", " 最大缩放值 " + maxZoom + " 设置缩放值 " + camera2Manager.getZoomProportion() );
            try {
     ...
     * @param requestBuilder
     */
    public void setAutoFocus(CaptureRequest.Builder requestBuilder) {
    ...
    private void setUpCameraOutputs(Activity activity, int width, int height) {
        ...
        try {
            ...
            for (String cameraId : manager.getCameraIdList()) {
                ...
                if (!Camera2Utils.matchCameraDirection(cameraCharacteristics, mCurrentDirection)) {
                    continue;
                }
                //存储流配置类
                ...
                Log.i(TAG, (mCurrentDirection == CameraCharacteristics.LENS_FACING_BACK ? "后" : "前") + " 摄像头 " + " 是否支持自动对焦 " + mAutoFocusSupported + " 获取到焦距的最大值 " + minimum_focus_distance + " 最大的缩放值 " + maxZoom);
                //对于静态图片，使用可用的最大值来拍摄。
                ...
                mCameraId = cameraId;
                Log.i("camera_log", " 根据相机的前后摄像头" + mCameraId + " 方向是：" + mCurrentDirection);
                return;
            }
        } catch (Exception e) {
            e.printStackTrace();
